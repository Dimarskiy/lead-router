...
## Slack-команды и интерактивность

Помимо обычных уведомлений Lead Router поддерживает интерактив:

- **Кнопки в сообщении о лиде**: `✅ Принял` / `❌ Не могу взять`. «Принял» помечает лид `accepted`. «Не могу взять» отклоняет лид и сразу переназначает следующему менеджеру (исключая всех, кто уже отказался).
- **Слэш-команда `/leadrouter`**:
  - `status` — твои pending-лиды и итоги дня
  - `pause [1h|30m]` — пауза (без аргумента — до конца дня); авто-возврат когда пауза истекает
  - `resume` — снова в очереди
  - `help`
- **Утренний отчёт**: cron-расписание (по умолчанию Пн-Пт в 9:00 в выбранной таймзоне). Метрики за вчера + Топ-3 менеджеров.
- **Эскалация**: после `escalation_threshold` переназначений алерт уходит тимлиду (`escalation_user_id`) или в общий канал.
- **SLA-алерт**: если pending-лид висит дольше `sla_hours` без касания, в `sla_alert_channel` уходит уведомление с тегом ответственного менеджера.

Полная инструкция по настройке Slack App — см. [`SLACK_SETUP.md`](./SLACK_SETUP.md).

### Дополнительные эндпоинты бэкенда

| Метод | Путь | Описание |
|---|---|---|
| POST | /slack/interactive | Обработчик кнопок (action_id `accept_lead`, `reject_lead`) |
| POST | /slack/command | Обработчик слэш-команды `/leadrouter` |
| GET | /api/queue | Очередь нераспределённых лидов |
| POST | /api/queue/distribute | Равномерно раскинуть очередь по активным менеджерам |
| POST | /api/queue/assign | Назначить конкретный лид конкретному менеджеру |
| DELETE | /api/queue/:lead_id | Убрать лид из очереди |
| GET | /api/schedules | Недельные расписания смен |
| PUT | /api/schedules/:manager_id | Сохранить расписание менеджера |
